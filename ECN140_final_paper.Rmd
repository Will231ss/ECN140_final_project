...
The contribution is threefold. First, we update the empirical estimates of the obesity-activity-diabetes interaction using the most recent BRFSS data (1.33M observations across three years). Second, we show that the comparison between the LPM and the logit specifications reveals a substantive disagreement about the *magnitude* of the absolute interaction: the LPM finds a 6.9 pp probability-scale interaction, while the same data fit with a logit — which freely estimates the over_obese × active term and finds it indistinguishable from zero on the log-odds scale, implying an approximately constant odds-ratio for activity across weight groups — yield an average probability-scale interaction of about 3.5 pp across the empirical covariate distribution. Third, we document that the cross-sectional gradient is similar across the 2021/2023/2024 waves.
...
Two specific data-quality issues were flagged in the cleaning audit. First, Kentucky (FIPS 21) and Pennsylvania (FIPS 42) did not meet BRFSS minimum requirements in 2023 and are absent from that year's public-use file (CDC, 2024). The state fixed effects absorb this mechanically. Second, the BRFSS PA Module containing the CDC physical-activity guideline variables (`_PAINDX3`, `_PAREC3`) was only fielded in 2023 across our three years. For the pooled headline analysis we therefore use the binary `_TOTINDA` ("any leisure-time activity in past 30 days"), available in all three years; we use the richer guideline variables in a 2023-only robustness check.
...
We choose the LPM as the headline specification for two reasons. First, the coefficients are interpretable in the natural units (percentage points of diabetes probability), making the magnitude directly comparable to clinical and policy benchmarks (Lecture 16). Second, the LPM allows a direct contrast with the multiplicative-form Poisson model used by (Abernethy et al., 2025), which is the precedent paper for our question.
...
Table 2 presents the four-step LPM build-up (Models 1-4) together with the logit (Model 5) and survey-weighted LPM (Model 6) estimates of the headline interaction model. Three patterns stand out.
...
First, the level effect of overweight/obesity is large and stable across Models 1-3, which estimate the *average* over/obese effect: 10.8 → 9.4 → 9.2 pp as controls and fixed effects are added. The coefficient jumps to 14.3 pp in Model 4 because that column introduces activity and the interaction simultaneously; 14.3 pp is now the over/obese effect *among the inactive*, a different estimand. The fact that the average over/obese effect barely moves between Models 2 and 3 (9.4 vs. 9.2 pp) indicates that geographic and temporal compositional effects do not explain most of the gradient.

Second, the interaction in the LPM is negative, large, and precisely estimated. The coefficient $\hat{\beta}_3 = -0.069$ (SE 0.003, t = -26.6, p < 0.001) means that being active is associated with a 6.9 pp lower over/obese marginal effect. Combined with the level effect of activity ($\hat{\beta}_2 = -0.020$), the *total* cross-sectional gap between active and inactive over/obese respondents is **-8.9 percentage points** versus **-2.0 percentage points** for a normal/under-weight respondent. We formally test the implied joint hypothesis $\beta_2 + \beta_3 = 0$ and reject it overwhelmingly (Wald F > 1000, p < 0.001).

Third — and this is the central methodological point — the logit on the *same data* yields a smaller probability-scale interaction than the LPM. The logit freely estimates the over/obese × active term and finds it indistinguishable from zero on the log-odds scale (-0.007, SE 0.018, p = 0.69). That implies an approximately constant odds-ratio of $\exp(-0.51) \approx 0.60$ for the association of activity with diabetes at both weight levels — but the constancy is an *estimated result*, not an assumption the model imposes. To make the LPM-vs-logit comparison concrete, Table 3 reports the four LPM-vs-logit predicted cell probabilities evaluated at a representative covariate cell (white respondent, mid-income, mid-education, California 2023; age, sex, and smoking at sample means). Both models predict similar within-cell probabilities, but their probability-scale interactions differ: the LPM gives a 6.9 pp interaction (= 8.96 pp − 2.05 pp), the logit gives 3.58 pp (= 6.16 pp − 2.58 pp) at this cell.
...
In summary, both models agree on sign but the LPM yields roughly twice the probability-scale interaction of the logit. Whether physical activity "attenuates" obesity therefore depends on functional form: the LPM matches the clinical question (absolute pp reductions for higher-baseline-risk patients); the logit matches the standard epidemiological summary (odds-ratio scaling). The survey-weighted LPM (column 6) recovers essentially the same interaction (-0.062 vs. -0.069 unweighted), so the result is not driven by unweighted oversampling.
...
Two specific limitations tied to this dataset. First, BMI and diabetes status are both self-reported, and CDC's validation work documents systematic underreporting of body weight in telephone interviews (Pierannunzi et al., 2013); if under-reporting is correlated with activity status, our LPM interaction could be partly artifactual. NHANES (which measures BMI and uses HbA1c) would allow a population-level measurement-error calibration, though it is an independent sample so the calibration cannot be done at the individual level. Second, listwise deletion on income drops ~18 pp of the sample at the SES-control step; a specification omitting income controls yields a very similar interaction (-0.067 vs. -0.069), but the non-random missingness pattern warrants the caveat.
