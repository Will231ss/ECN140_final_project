...
This paper asks whether physical activity attenuates the obesity-diabetes association using 1.33 million observations from the 2021, 2023, and 2024 waves of the Behavioral Risk Factor Surveillance System (BRFSS). The unit of observation is the adult respondent. We estimate linear probability and logit models with state and year fixed effects and state-clustered standard errors, focusing on the interaction between overweight/obesity and leisure-time physical activity in predicting self-reported diabetes. In the linear probability model the interaction is -6.9 percentage points (t = -26.6): the active-vs-inactive gap is -8.9 pp among over/obese respondents versus -2.0 pp among normal-weight ones. Fit to the same data, a logit returns a near-zero log-odds interaction (-0.007, p = 0.69), which averages to only about -3.5 pp on the probability scale. The two models agree on sign but disagree on magnitude by roughly a factor of two; the LPM is fitting a steeper-than-logistic additive gradient. Robustness checks using the 2023 CDC physical-activity guideline, continuous BMI, survey weights, and pre-diabetes all reinforce the pattern, which is stable across the three waves. We interpret all estimates as associations, not causal effects.
